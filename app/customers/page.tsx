  ...
  return (
    <div className="space-y-6 relative h-full">
      <PageHeader 
        ...
        description="Unified corporate directory, accounting ledgers, and credit limit tracking."
        actions={
              ...
              <table className="w-full min-w-[700px] text-left border-collapse">
                ...
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50 text-slate-500 text-xs font-bold uppercase tracking-wider">
                    ...
                    <th className="px-6 py-3.5 text-center">Credit Term</th>
                    <th className="px-6 py-3.5 text-right">Total Orders</th>
                    <th className="px-6 py-3.5 text-right">Receivable Balance</th>
                  </tr>
                </thead>
                      ...
                      >
                        <td className="px-6 py-4">
                          <div>
                            ...
                            <span className="text-xs text-slate-400 block mt-0.5">ID: {c.id} | Tax ID: {c.taxId}</span>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <div>
          ...
          <Card className="w-full lg:w-96 shrink-0 shadow-lg border-blue-100/60 sticky top-20">
            <CardHeader className="border-b border-slate-100 pb-4 flex flex-row justify-between items-start">
              <div className="space-y-1">
                <span className="text-[10px] uppercase font-bold text-blue-600 tracking-wider">Customer Profile</span>
            ...
            <CardContent className="pt-6 space-y-6">
              ...
              {/* Tax Details */}
              <div className="space-y-3">
                <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Financial Setup</h4>
                ...
                <div className="divide-y divide-slate-100 border border-slate-100 rounded-lg bg-slate-50/30 overflow-hidden text-xs">
                  <div className="flex justify-between p-2.5">
                    <span className="text-slate-500 font-semibold">Tax ID</span>
                    <span className="font-bold text-slate-900">{selectedCustomer.taxId}</span>
                  </div>
                  <div className="flex justify-between p-2.5">
                    <span className="text-slate-500 font-semibold">Credit Term</span>
                  ...
                  <div className="flex justify-between p-2.5">
                    ...
                    <span className="font-bold text-slate-900">{selectedCustomer.totalOrders} items</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1">
                  <History className="w-3.5 h-3.5 text-blue-600" /> Recent Invoices
                </h4>
                    ...
                    customerInvoices.map((inv) => (
                      <div key={inv.id} className="p-2.5 flex items-center justify-between hover:bg-slate-50 transition-colors">
                        <div>
                          ...
                          <span className="text-[10px] text-slate-400 block mt-0.5">Due: {inv.dueDate}</span>
                        </div>
                        <div className="text-right">
                          ...
                          <span className="text-[10px] block mt-0.5">
                            {inv.status === 'Paid' ? (
                              ...
                              <span className="text-rose-500 font-semibold">Due: {formatCurrency(inv.outstandingAmount)}</span>
                            )}
                          </span>
                        </div>
                      </div>
